#!/usr/bin/env python3
"""
MBO Ridge Walk-Forward with TRAILING-WINDOW Z-SCORED FEATURES.

Tests real-time viability: uses only PAST + CURRENT samples for z-scoring
(not full-day). If signal survives, the strategy is implementable in live trading.

Tests K=30 (30 min), K=60 (1 hr), K=120 (2 hr) trailing windows.
4 features (drops unstable volume_imbalance).

Usage:
    python scripts/ridge_walkforward_mbo_trailing.py
"""
...
OUTPUT_DIR = "outputs/ridge_walkforward_mbo_trailing"
TARGET_HORIZON_IDX = 7  # H=60

# 4 features (dropped unstable volume_imbalance, CV=12.85)
FEATURES = {
    "spread_bps": 42, "total_ask_volume": 44,
    "true_ofi": 84, "depth_norm_ofi": 85,
}
TRAILING_WINDOWS = [30, 60, 120]
MIN_TRAILING = 30  # skip samples with fewer than this many trailing observations
...
def load_all_data(loader, dates, feature_indices, trailing_K=60, min_trailing=30):
    """Load all days with TRAILING-WINDOW z-scored features.

    Uses only PAST + CURRENT samples for z-scoring (real-time implementable).
    Samples with fewer than min_trailing observations are excluded (NaN → filtered).

    Args:
        trailing_K: Number of trailing samples for z-score window.
        min_trailing: Minimum samples required before z-scoring starts.
    """
    day_data = []
    total_skipped = 0
    for d in dates:
        ...
        # Trailing-window z-score: only uses PAST + CURRENT samples
        feat_z = np.full_like(feat_raw, np.nan)
        for i in range(min_trailing - 1, N):
            start = max(0, i - trailing_K + 1)
            trail = feat_raw[start:i + 1, :]
            t_mu = np.mean(trail, axis=0)
            t_sigma = np.std(trail, axis=0)
            t_sigma[t_sigma < 1e-10] = 1.0
            feat_z[i] = (feat_raw[i] - t_mu) / t_sigma

        # Keep only valid (non-NaN) samples
        valid = np.all(np.isfinite(feat_z), axis=1) & np.isfinite(lab)
        n_valid = int(valid.sum())
        n_skipped = N - n_valid
        total_skipped += n_skipped

        day_data.append({
            "date": d, "features": feat_z[valid], "labels": lab[valid],
            "n": n_valid, "n_skipped": n_skipped,
        })
    return day_data, total_skipped
...
def main():
    ...
    feature_names = [n for n, _ in sorted(FEATURES.items(), key=lambda x: x[1])]

    # === Analysis 1: Walk-Forward across trailing windows × alphas ===
    log("Analysis 1: Walk-forward Ridge (trailing windows)...")
    wf_results = {}
    best_config = None
    best_stability = -999

    for K in TRAILING_WINDOWS:
        log(f"  Loading train data with trailing K={K}...")
        train_data_K, skipped = load_all_data(
            train_loader, train_dates, all_indices,
            trailing_K=K, min_trailing=MIN_TRAILING,
        )
        log(f"    Skipped {skipped} samples ({skipped / (245 * len(train_dates)) * 100:.1f}%)")

        for alpha in ALPHAS:
            key = f"K{K}_alpha_{alpha}"
            log(f"    {key}...")
            summary, betas = walk_forward(train_data_K, alpha, feature_names)
    ...
    # Extract best K and alpha from best_config
    parts = best_config.split("_alpha_")
    best_K = int(parts[0][1:])  # "K60" → 60
    best_alpha = float(parts[1])

    # Reload best K data for subsequent analyses
    best_train_data, _ = load_all_data(
        train_loader, train_dates, all_indices,
        trailing_K=best_K, min_trailing=MIN_TRAILING,
    )
    best_feature_names = feature_names

    # (keep variable name for compatibility with rest of script)
    best_subset_name = f"K{best_K}"

    # === Analysis 2: Val/Test OOS ===
    log("Analysis 2: Val/Test OOS...")
    val_data_best, _ = load_all_data(
        val_loader, val_dates, all_indices,
        trailing_K=best_K, min_trailing=MIN_TRAILING,
    )
    test_data_best, _ = load_all_data(
        test_loader, test_dates, all_indices,
        trailing_K=best_K, min_trailing=MIN_TRAILING,
    )
    oos_results = evaluate_oos(best_train_data, {"val": val_data_best, "test": test_data_best},
                                best_alpha, best_feature_names)

    # === Analysis 4: Single-Feature Baseline ===
    log("Analysis 4: Single-feature baseline (spread_bps)...")
    spread_idx_in_features = all_indices.index(42)
    sf_data = [{"date": d["date"],
                 "features": d["features"][:, spread_idx_in_features:spread_idx_in_features + 1],
                 "labels": d["labels"], "n": d["n"]} for d in best_train_data]
    ...
    pnl = compute_walkforward_pnl(None, best_train_data, best_alpha, all_indices)
    ...
    results = {
        ...
        "best_trailing_K": best_K,
        "trailing_windows_tested": TRAILING_WINDOWS,
