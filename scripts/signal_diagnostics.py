#!/usr/bin/env python3
"""
Signal diagnostics for off-exchange spread_bps threshold strategy.

7 diagnostics to determine GO/NO-GO before committing to model training:
  1. Multi-horizon threshold backtest (H=10,20,30,60)
  2. Val/Test OOS threshold backtest
  3. Regime robustness (up/down days, halves, drift-adjusted)
  4. Trade return distribution analysis
  5. Cumulative P&L curve + drawdown
  6. Statistical power calculation
  7. Feature conditioning (session_progress)

Usage:
    python scripts/signal_diagnostics.py
    python scripts/signal_diagnostics.py --export-dir /path/to/export
"""
...
DEFAULT_OUTPUT_DIR = "outputs/signal_diagnostics"
HORIZONS = (1, 2, 3, 5, 10, 20, 30, 60)
TARGET_HORIZONS = [3, 4, 5, 6, 7]  # indices for H=5,10,20,30,60
SPREAD_BPS_IDX = 12
SESSION_PROGRESS_IDX = 31
BBO_UPDATE_RATE_IDX = 15
...
def load_split_data(loader, dates):
    """Load per-day spread_bps, session_progress, bbo_update_rate, and labels."""
    data = {}
    for d in dates:
        b = loader.load_day(d)
        data[d] = {
            "spread": np.asarray(b.sequences[:, -1, SPREAD_BPS_IDX], dtype=np.float64),
            "session": np.asarray(b.sequences[:, -1, SESSION_PROGRESS_IDX], dtype=np.float64),
            "bbo_rate": np.asarray(b.sequences[:, -1, BBO_UPDATE_RATE_IDX], dtype=np.float64),
